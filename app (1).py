...
# --- 8. AI Forecasting (ມີຄ່າສະເລ່ຍ 7 ວັນ ແລະ % ແນວໂນ້ມ ເປະໆ) ---
    ...
    if len(daily_sales) < 7:
        ...
        # ຄຳນວນຄ່າສະເລ່ຍ 7 ວັນຜ່ານມາ
        avg_past_7 = daily_sales['total_sales'].tail(7).mean()
        
        # AI ພະຍາກອນ 7 ວັນຂ້າງໜ້າ
        ...
        # ສ້າງ DataFrame ສຳລັບສະແດງກາຟ (ກວດສອບຊື່ Column ໃຫ້ຕົງກັນ)
        f_df = pd.DataFrame({
            'ວັນທີ': [(last_date + timedelta(days=i)).date() for i in range(1, 8)],
            'ຍອດຄາດຄະເນ (฿)': [round(v, 2) for v in forecast_values]
        })
        
        # ແກ້ໄຂແລ້ວ: text="ຍອດຄາດຄະເນ (฿)" ໃຫ້ຕົງກັບຊື່ Column ໃນ f_df
        st.subheader("📅 ລາຍລະອຽດການຄາດຄະເນ 7 ວັນລ່ວງໜ້າ")
        st.plotly_chart(px.line(f_df, x='ວັນທີ', y='ຍອດຄາດຄະເນ (฿)', 
                                markers=True, 
                                text='ຍອດຄາດຄະເນ (฿)', # <--- ບ່ອນທີ່ເຄີຍຜິດ
                                title="ເສັ້ນສະແດງແນວໂນ້ມຍອດຂາຍ 7 ວັນລ່ວງໜ້າ"), 
